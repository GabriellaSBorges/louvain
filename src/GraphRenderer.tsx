...
import { applyLouvainRecursively } from './LouvainRecursively';
...
const GraphRenderer = (props: Props) => {
    ...
    useEffect(() => {
        ...
        const { graph: finalGraph, communityHierarchy } = applyLouvainRecursively(graph, 3);
        const thirdLevelCommunities = communityHierarchy[2];
        ...
        for (let i = 0; i < thirdLevelCommunities.count; i++){
            colors[i] = Math.random() * 16777215; 
        }

        // Atribui cores e outros atributos aos nós com base em suas comunidades
        for (let element in thirdLevelCommunities) {
            graph.mergeNodeAttributes(element, {
                color: "#" + Math.floor(colors[thirdLevelCommunities[element]] + graph.getNodeAttribute(element,'size')).toString(16),
                borderColor: "white",
                community: thirdLevelCommunities[element]
            });
        }
